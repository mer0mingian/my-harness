...
This package ships static SpecKit assets (markdown commands and templates) plus Python helper scripts, hook handlers, and library code. Tests follow the layer:

- **Python logic** (`scripts/`, `lib/`, `hooks/handlers/`) — full pytest coverage of behaviour.
- **Command markdown** (`commands/*.md`) — YAML frontmatter validity only (machine-readable metadata). No assertions on prose, headings, or wording.
- **Templates** (`templates/*.md`) — YAML frontmatter validity only. No structural prose tests.

End-to-end / dogfood runs verify that an agent following a command markdown file produces the expected artifacts. Section drift in prose is caught by PR review, not by unit tests. Rationale: a `.md` file interpreted by an LLM has no behaviour to regress against; asserting `"## Step 5" in content` proves nothing about whether the agent will follow Step 5.
...
**Acceptance criteria:**
- [ ] Both templates have valid YAML frontmatter (machine-readable metadata only)
- [ ] All required sections present (per diagram) — verified by PR review, not unit tests
...
**Acceptance criteria:**
- [ ] Both templates have valid YAML frontmatter (machine-readable metadata only)
- [ ] ADR template has sections for 3 alternatives (each with C1 + C2 diagram blocks) — verified by PR review
- [ ] Solution Design template has Decomposition/Dependency/Interface/Data views — verified by PR review
...
- [ ] Solution Design artifact has all 4 views (verified by E2E run + PR review)
- [ ] Artifact validation runs post-generation (warn, not block)
...
## Test Strategy Implementation Note

The Test Strategy section above reflects a decision made on 2026-05-08 and implemented in commit `04d0fa2`: structural markdown unit tests (6 files, ~143 test cases that asserted heading strings in `commands/*.md` and `templates/*.md`) were removed from the test suite. The remaining 244 Python logic tests (in `scripts/`, `lib/`, `hooks/handlers/`) continue to pass.

The verification mechanism for command/template content is now:

- YAML frontmatter validity — checked by yamllint or equivalent linter pass
- Behavioural correctness — checked by end-to-end / dogfood runs of each command
- Prose structure — checked by PR review (no automated test)

Story-point impact: S7d (`/speckit.multi-agent.solution-design` command) reduced from 5 pts to 3 pts since the structural test file authoring scope (~40–50% of the original estimate) is removed.
