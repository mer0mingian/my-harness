...
|-------|---------|-------------------|-------------|-------------|--------|
| **0. Orchestration** | `/speckit.matd.execute` | Workflow state | orchestrator | matd-workflow | **PROPOSED: Replace with workflow YAML** |
| **1. Product Spec** | `/speckit.matd.specify-product-brief` | `docs/features/{id}-prd.md` | pm + grill-me | general-grill-me | **PROPOSED: Remove (use SpecKit + skill)** |
| **2. Test Strategy** | `/speckit.matd.specify-test-strategy` | `docs/testing/{id}-strategy.md` | pm + grill-me | general-grill-me, dev-tdd | **PROPOSED: Remove (move to constitution)** |
| **3. Solution Design** | `/speckit.matd.specify-solution-design` | `docs/design/{id}-solution.md` + ADR | arch + pm | arch-c4-*, general-grill-me | **PROPOSED: Remove (use /speckit.plan)** |
| **4. ADR Creation** | `/speckit.matd.specify-adr` | `docs/adrs/{num}-{title}.md` | arch + grill-me | arch-design-principles | **PROPOSED: Simplify (thin wrapper)** |
| **5. Test Generation (RED)** | `/speckit.matd.test` | `docs/features/{id}-test-design.md` + test files | **test-author** | stdd-test-author-constrained | **Keep - Simplify implementation** |
| **6. Implementation (GREEN)** | `/speckit.matd.implement` | Source code files | **developer** | dev-tdd, verification-before-completion | **Keep - Simplify to single-phase** |
| **7. Parallel Review** | `/speckit.matd.review` | `docs/reviews/{id}-arch.md` + `{id}-code.md` | **@check (arch)** + **@simplify (code)** | review-check-correctness, review-simplify-complexity | **Keep as-is (unique value)** |
| **8. Commit & Summary** | `/speckit.matd.commit` | `docs/features/{id}-summary.md` + git commit | validator | verification-before-completion | **Keep - Add doc update hooks** |

#### Key Subagents (Preserved in Simplification)

| Subagent | Role | Current Usage | After Simplification |
|----------|------|---------------|---------------------|
| **test-author** | Generate failing tests (RED state) | `/speckit.matd.test` | ✅ Preserved - Used in simplified test command |
| **developer** | Implement feature (GREEN state) | `/speckit.matd.implement` | ✅ Preserved - Used in single-phase implement |
| **@check** | Architecture review | `/speckit.matd.review` | ✅ Preserved - No changes to review command |
| **@simplify** | Code quality review | `/speckit.matd.review` | ✅ Preserved - No changes to review command |
| orchestrator | Workflow coordination | `/speckit.matd.execute` | ⚠️ Replaced by SpecKit workflow engine |
| pm + grill-me | Requirements gathering | `specify-*` commands | ⚠️ Replaced by direct skill/command invocation |
| arch + pm | Solution design | `specify-solution-design` | ⚠️ Replaced by enhanced `/speckit.plan` |

**Important**: The core MATD subagent architecture (test-author, developer, @check, @simplify) **remains unchanged**. Only orchestration and specification discovery commands are affected.

### 2.2 Simplified Workflow (After Implementation)

```mermaid
graph LR
    A[SpecKit Phases 0-4] --> B[/speckit.matd.test<br/>Subagent: test-author]
    B --> C[/speckit.matd.implement<br/>Subagent: developer]
    C --> D[/speckit.matd.review<br/>Subagents: @check + @simplify]
    D --> E[/speckit.matd.commit<br/>+ PR merge hooks]
    E --> F[Auto: C4 + CGC update<br/>on main/live merge]
```
...
**What stays the same**:
- test-author subagent generates tests
- developer subagent implements features
- @check + @simplify subagents perform parallel review
...
**Subagents used**: pm + grill-me (generic interviewer, not MATD-specific)
...
- `/speckit.matd.specify-adr` - Thin wrapper around arch skills

**Issue**: Reimplements functionality available in SpecKit core  
**Subagents used**: ✅ **test-author, developer remain unchanged**
...
**Subagents used**: @check + @simplify (core MATD value)
...
| Simplification | Test-Author | Developer | @check + @simplify | Impact |
|---------------|-------------|-----------|-------------------|---------|
...
**Conclusion**: All MATD subagents (test-author, developer, @check, @simplify) continue to function identically after simplification. Changes only affect orchestration logic and SpecKit integration patterns.
...
#### Command #2: `/speckit.matd.specify-product-brief`
...
**Why remove**:
- Thin wrapper around `general-grill-me` marketplace skill
- Same result achievable with: `/speckit.specify feat-123` + invoke `@grill-me`
- SpecKit Phase 1 (Specify) already handles this

**Subagent impact**: ✅ None - same grill-me skill used

**Replacement**:
```bash
# Instead of: /speckit.matd.specify-product-brief feat-123
# Use: /speckit.specify feat-123 --interactive
# During specification, invoke: @grill-me
```
...
#### Command #4: `/speckit.matd.specify-solution-design`
...
**Why remove**:
- Overlaps with `/speckit.plan` (SpecKit Phase 3)
- C4 generation handled by `arch-c4-*` marketplace skills
- ADR creation duplicates `/speckit.matd.specify-adr`

**Subagent impact**: ✅ None - same arch skills used

**Replacement**:
```bash
# Instead of: /speckit.matd.specify-solution-design feat-123
# Use: /speckit.plan feat-123 --include-architecture --compare-approaches 3
# During planning, invoke: @arch-c4-container @arch-c4-component
```
...
### 4.2 Summary of Proposed Removals

| Command | Lines | Reason | Replacement | Subagent Impact |
|---------|-------|--------|-------------|-----------------|
| execute | 300+ | Duplicates workflow system | SpecKit workflow YAML | ✅ None |
| specify-product-brief | ~200 | Wrapper around grill-me | `/speckit.specify` + skill | ✅ None |
| specify-test-strategy | ~300 | Project-level, not feature | Constitution | ✅ None |
| specify-solution-design | ~200 | Overlaps with /speckit.plan | `/speckit.plan` + skills | ✅ None |
| **Total** | **~1,000 lines** | **4 commands** | **SpecKit core + skills** | **✅ No impact** |

**Complexity removed**: 20 SP  
**Effort to remove**: 10 SP  
**ROI**: 200%
...
#### harness-tooling
- **MATD Simplification** (24 SP): Remove 4 commands, simplify 4 commands
...
# (Check logs for test-author, developer, @check, @simplify)
```

##### Task 1.2: Remove Specification Commands (7 SP)
...
```markdown
## Requirements Gathering (replaces specify-product-brief)
```bash
/speckit.specify feat-123 --interactive
# During specification: @grill-me
```

## Test Strategy (replaces specify-test-strategy)
...
## Solution Design (replaces specify-solution-design)
```bash
/speckit.plan feat-123 --include-architecture --compare-approaches 3
# During planning: @arch-c4-container @arch-c4-component
```
```
...
! test -f spec-kit-multi-agent-tdd/commands/specify-product-brief.md
! test -f spec-kit-multi-agent-tdd/commands/specify-test-strategy.md
! test -f spec-kit-multi-agent-tdd/commands/specify-solution-design.md

# Verify USER-GUIDE updated
grep -q "Requirements Gathering" spec-kit-multi-agent-tdd/USER-GUIDE.md
```

#### Phase 2: Simplify Core Commands (14 SP)

**Important**: All changes preserve subagent architecture (test-author, developer, @check, @simplify)
...
# ✅ PRESERVE: test-author subagent invocation
invoke_subagent test-author --context "$test_design_artifact"
```
...
# Verify test-author subagent called
grep "Invoking test-author subagent" <(specify logs)
```
...
3. **✅ Preserve developer subagent invocation**
...
# ✅ PRESERVE: developer subagent invocation
invoke_subagent developer \
  --context "$test_files" \
  --mode "tdd-green"
...
# Verify developer subagent called
grep "Invoking developer subagent" <(specify logs)
...
##### Task 2.4: Simplify matd.specify-adr (2 SP)

**File**: `spec-kit-multi-agent-tdd/commands/specify-adr.md`

**Changes**: Thin wrapper around SpecKit template + arch skill

**After** (~50 lines):
```bash
specify artifact render adr \
  --template adr-template.md \
  --var "decision_title=$title" \
  --var "adr_number=$number"

echo "Invoke @arch-design-principles for content"
```
...
- ✅ Clarified subagent preservation (test-author, developer, @check, @simplify)
...
Before proceeding with Sprint 1, please confirm:

- [ ] **Command #1 removal**: `/speckit.matd.execute` → Replace with workflow YAML
- [ ] **Command #2 removal**: `/speckit.matd.specify-product-brief` → Use SpecKit + skill
- [ ] **Command #3 removal**: `/speckit.matd.specify-test-strategy` → Move to constitution
- [ ] **Command #4 removal**: `/speckit.matd.specify-solution-design` → Use /speckit.plan

**Subagent verification**: Confirm that test-author, developer, @check, @simplify preservation is acceptable

**Auto-docs trigger**: Confirm PR merge trigger (not every commit) is acceptable
...
#### Files Pending Removal (Awaiting Approval)
...
specify logs | grep -E "test-author|developer|@check|@simplify"
